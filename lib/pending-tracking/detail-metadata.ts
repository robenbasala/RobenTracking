...
export async function buildDetailResponse(
  ...
  const fieldsMeta = fieldsMetaRaw.filter(
    (f) => f.FieldName !== TRACKING_ITEM_ID_FIELD_NAME
  )
  ...
  const hasCustom = fieldsMeta.some((f) => f.SourceType === "Custom")
  const customMap = hasCustom
    ? await fetchCustomValueRows(pool, trackingItemId)
    : new Map<number, TfRow>()

  const fields: UnifiedDetailFieldRow[] = fieldsMeta.map((fm) => {
    ...
    if (fm.SourceType === "BaseTable") {
      ...
      if (fm.DataType.toLowerCase() === "dropdown" && row) {
        dropdownOptionId = row.DropdownOptionId ?? null
      }
    }
    const opts =
