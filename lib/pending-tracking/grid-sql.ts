...
export function buildGridColumnMetadataFromFields(
  ...
  return fields
    ...
    .map((f) => ({
      ...
      isEditable: f.IsEditable && !f.IsSystemField,
    }))
...
function sortKeyToPascal(
  ...
  const camelMatch = fields.find(
    (f) => pascalToCamelKey(f.FieldName) === sortBy
  )
...
export function validateSortColumnPascal(
  ...
  if (pascal && isSafeFieldNameAlias(pascal)) {
    return bracketQuoteIdentifier(pascal)
  }
  return bracketQuoteIdentifier(
    fields[0]?.FieldName ?? TRACKING_ITEM_ID_FIELD_NAME
  )
}
...
export function buildGridDataSql(
  ...
  const hasCustom = fields.some((f) => f.SourceType === "Custom")
