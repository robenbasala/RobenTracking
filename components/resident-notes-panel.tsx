...
export function ResidentNotesPanel({
  ...
  async function addNote() {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/pending-tracking/${trackingItemId}/notes`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ companyId, body, noteType: "CaseNote" }),
      })
    ...
    try {
      const res = await fetch(
        `/api/pending-tracking/${trackingItemId}/notes/${noteId}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ companyId, body: editBody.trim() }),
        }
      )
