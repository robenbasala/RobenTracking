...
export default function AdminFieldsPage() {
  ...
  async function deleteField(f: AdminField) {
    if (!f.deletable) return
    const msg =
      f.fieldKind === "calculated"
        ? `Delete calculated field "${f.displayName}" (${f.fieldName})?`
        : `Delete custom field "${f.displayName}" (${f.fieldName})? Stored values must be empty — this cannot be undone.`
    ...
    try {
      ...
      if (editing?.fieldMetadataId === f.fieldMetadataId) setEditing(null)
      await load()
    } catch (e) {
                                ...
                                title={
                                  f.deletable
                                    ? f.fieldKind === "calculated"
                                      ...
                                      : "Delete field (no stored values)"
                                    : f.fieldKind === "calculated"
                                      ? "Cannot delete"
                                      : f.sourceType !== "Custom"
                                        ? "Base-table fields cannot be deleted"
                                        : "Has stored values — clear them first"
                                }
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div
            role="dialog"
            aria-modal="true"
            className="bg-surface-container-lowest flex max-h-[90vh] w-full max-w-md flex-col overflow-hidden rounded-2xl border border-border shadow-lg"
          >
            ...
            {editing.fieldKind === "calculated" && (
              <p className="border-tertiary bg-tertiary/5 text-on-surface mb-3 rounded-md border-l-4 px-3 py-2 text-xs">
                This is a <strong>calculated</strong> field. Values are computed at
                runtime. You can change labels and visibility here; to change the
                formula, use{" "}
                <code className="rounded bg-muted px-1">PATCH /api/admin/field-metadata/:id</code>{" "}
                with <code className="rounded bg-muted px-1">formulaDefinition</code>.
                {editing.formulaDefinition != null && (
                  <pre className="border-input bg-muted/40 mt-2 max-h-32 overflow-auto rounded border p-2 font-mono text-[10px] leading-relaxed">
                    {JSON.stringify(editing.formulaDefinition, null, 2)}
                  </pre>
                )}
              </p>
            )}
