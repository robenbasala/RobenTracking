...
export default function TrackingPage() {
  ...
  const loadViewTypes = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/pending-tracking/view-types?${params}`, {
        cache: "no-store",
        signal: ac.signal,
      })
      ...
      setTabsError(
        aborted
          ? `Request timed out after ${VIEW_TYPES_FETCH_TIMEOUT_MS / 1000}s. The database may be unreachable — check TRACKING_DB_CONNECTION_STRING and that SQL Server allows connections from this machine.`
          : e instanceof Error
            ? e.message
  ...
  useEffect(() => {
    async function loadFacilities() {
      try {
        const params = new URLSearchParams()
        params.set("companyId", String(companyId))
        const res = await fetch(`/api/pending-tracking/facilities?${params}`, {
          cache: "no-store",
        })
  ...
  const handleExport = useCallback(() => {
    ...
    window.open(`/api/pending-tracking/export?${params}`, "_blank")
