...
async function loadMetadataForFields(
  ...
  for (const r of result.recordset) {
    const fk =
      (r as { FieldKind?: string }).FieldKind != null
        ? String((r as { FieldKind?: string }).FieldKind)
        : "regular"
    const fj = (r as { FormulaDefinitionJson?: unknown }).FormulaDefinitionJson
    map.set(r.FieldName, {
      ...r,
